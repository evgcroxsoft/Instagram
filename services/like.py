...
class LikeService:
    async def create_like(self, db, session_data, post, like_status):
        ...
        if post_in_db is not None and post_in_db.status == PostStatus.VISIBLE:
            ...
            subscriber = (
                db.query(Subscribe)
                .filter_by(account_nickname=me, subscriber=post_in_db.account_nickname)
                .first()
            )
            if subscriber is not None and subscriber.status == SubcribeStatus.OK:

                like = (
                    db.query(Like).filter_by(account_nickname=me, post_id=post).first()
                )
                if like is None:
                    like = Like(
                        account_nickname=me,
                    ...
                    raise HTTPException(
                        ...
                        detail="Your like already exists",
                    )
            else:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="You have no rights to add like",
                )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Post {post} does not exists",
            )
    ...
    async def delete_like(self, db, session_data, post):
        ...
        if like is not None:
            ...
            raise HTTPException(
                ...
                detail=f"Like for post:{post} was deleted successfully",
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Post:{post} was not found",
            )
