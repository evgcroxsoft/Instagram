...
from models.enum import LikeStatus, PostStatus, SubcribeStatus
...
class PostService:
    ...
    async def get_all_my_posts(self, session_data, db, status):
        ...
        result = [
            ...
            for post in posts
            if post.status == status or status is None
        ]
        return result
    ...
    async def delete_post(self, session_data, db, id):
        nickname = await account_services.current_nickname(session_data)
        specify_post = (
            db.query(Post).filter_by(account_nickname=nickname, id=id).first()
        )
        if specify_post is not None:
            ...
            raise HTTPException(
                ...
                detail=f"Post:{id} was deleted successfully",
            )
        else:
            raise HTTPException(
    ...
    async def get_all_posts(self, session_data, db):
        ...
        subscribers = (
            db.query(Subscribe)
            .filter_by(account_nickname=me, status=SubcribeStatus.OK)
            .all()
        )
        all_posts = [
            db.query(Post)
            .filter_by(account_nickname=post.subscriber, status=PostStatus.VISIBLE)
            .order_by(Post.created_at.desc(), Post.updated_at.desc())
            .all()
            for post in subscribers
        ]
        result = [
            PostPaginationScheme(
                id=post.id,
                description=post.description,
                media=post.media,
                likes=db.query(Like)
                .filter_by(post_id=post.id, status=LikeStatus.LIKE)
                .count(),
                dislikes=db.query(Like)
                .filter_by(post_id=post.id, status=LikeStatus.DISLIKE)
                .count(),
            )
