...
class UserService:
    ...
    async def delete_user(self, db, email):
        ...
        db.commit()
        raise HTTPException(
            status_code=status.HTTP_200_OK, detail="Host was deleted successfully"
        )
    ...
    async def get_current_is_active_user(
        db: Session = Depends(get_db),
        current_user: UserRetrieveScheme = Depends(get_current_user),
    ):
