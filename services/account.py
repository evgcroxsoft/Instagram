...
class AccountService:
    ...
    async def create_account(self, schema, current_user, db):
        ...
        if current_user.account != []:
            if len(current_user.account) < AccountService.COUNTER:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="limit accounts"
                )
    ...
    async def get_account(self, db, session_data, current_user):
        ...
        account = (
            db.query(Account)
            .filter_by(user_id=current_user.id, nickname=nickname)
            .first()
        )
        return account
    ...
    async def delete_account(self, db, session_data, current_user):
        ...
        raise HTTPException(
            ...
            detail=f"{nickname} account was deleted successfully",
        )
    ...
    async def get_all_accounts(self, current_user):
        if current_user.account != []:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Please create account"
            )
    ...
    async def get_account_with_post(self, nickname, db):
        ...
        for account in data:
            if account.nickname.startswith(nickname):
                ...
                post_in_db = (
                    db.query(Post)
                    .filter_by(
                        account_nickname=account.nickname, status=PostStatus.VISIBLE
                    )
                    .all()
                )
