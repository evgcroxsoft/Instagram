...
class SubscriberService:
    async def create_subscribe(self, session_data, db, nickname: str) -> subscribe:
        ...
        if await self.is_nickname_exist(db, nickname) is True:
            if await self.is_subscribe_exist(db, session_data, nickname) is False:
                subscribe = Subscribe(
                    account_nickname=await account_services.current_nickname(
                        session_data
                    ),
                    subscriber=nickname,
                ...
                raise HTTPException(
                    ...
                    detail="You have already sent request before",
                )
    ...
    async def i_subscribed(self, session_data, db):
        ...
        subscribers = (
            db.query(Account, Subscribe)
            .join(Subscribe)
            .filter_by(account_nickname=nickname)
            .all()
        )
        result = [
            {
    ...
    async def my_subscribers(self, session_data, db, status):
        ...
        result = [
            ...
            for account, subscriber in subscribers
            if subscriber.status == status or status is None
        ]
    ...
    async def update_subscriber_status(self, session_data, db, nickname, status):
        ...
        if (
            await self.is_nickname_exist(db, nickname) is True
            and await self.is_subscribe_exist(db, session_data, nickname) is False
        ):
            subscriber = (
                db.query(Subscribe)
                .filter_by(subscriber=me, account_nickname=nickname)
                .first()
            )
    ...
    async def delete_i_subscribed(self, session_data, db, nickname):
        ...
        subscriber = (
            db.query(Subscribe)
            .filter_by(account_nickname=me, subscriber=nickname)
            .first()
        )
        if subscriber is not None:
            ...
            raise HTTPException(
                ...
                detail=f"Subscriber:{nickname} was deleted successfully",
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Subscriber:{nickname} was not found",
            )
    ...
    async def is_nickname_exist(self, db, nickname: str) -> bool:
        ...
        for account in data:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Please check! {nickname} doesn't exists",
            )
