...
from schemas.user import BaseUserScheme, UserCreateScheme, UserRetrieveScheme, UserUpdateScheme
...
@router.post('/me', response_model=BaseUserScheme, status_code=status.HTTP_201_CREATED)
...
async def update_me(schema: UserUpdateScheme, current_user: UserRetrieveScheme = Depends(user_services.get_current_is_active_user), db: Session = Depends(get_db)):
