...
@router.post('/subscribe', response_model= SubscribeRetrieveScheme, status_code=status.HTTP_201_CREATED, dependencies=[Depends(cookie)])
...
@router.get('/subscribe/me', status_code=status.HTTP_200_OK, dependencies=[Depends(cookie)])
...
@router.get('/subscribe/to_me', status_code=status.HTTP_200_OK, dependencies=[Depends(cookie)])
...
@router.put('/subscribe', status_code=status.HTTP_201_CREATED, dependencies=[Depends(cookie)])
async def update_subscriber_status(
                            ...
                            session_data: SessionData = Depends(verifier),
                            db: Session = Depends(get_db)):
    return await subscriber_services.update_subscriber_status(session_data, db, nickname, status)


@router.delete('/subscribe/me', status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(cookie)])
async def i_subscribed(     
                            nickname: str,
                            current_user: UserRetrieveScheme = Depends(user_services.get_current_is_active_user), 
                            session_data: SessionData = Depends(verifier),
