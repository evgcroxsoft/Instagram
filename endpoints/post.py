...
async def get_all_my_posts(
    ...
    return paginate(
        await post_services.get_all_my_posts(session_data, db, status), params
    )


@router.get(
    "/post/{id}", status_code=status.HTTP_200_OK, dependencies=[Depends(cookie)]
)
...
@router.delete(
    "/post/{id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(cookie)]
)
